...
#             phase = torch.cumsum(rad_values, dim=1) * 2 * np.pi
            scale_factor_down = float(1.0 / self.upsample_scale)
...
#             cumsum_shift[:, 1:, :] = tmp_over_one_idx * -1.0
            ...
            phase = torch.cumsum(rad_values, dim=1) * 2 * np.pi
            scale_factor_up = float(self.upsample_scale)
...
class Generator(torch.nn.Module):
    def __init__(self, style_dim, resblock_kernel_sizes, upsample_rates, upsample_initial_channel, resblock_dilation_sizes, upsample_kernel_sizes, gen_istft_n_fft, gen_istft_hop_size, grad_checkpoint=False):
        ...
        self.dummy_tensor = torch.zeros(1, requires_grad=True)
        self._compiled = False
    
    # def compile_resblocks(self):
    #     if self._compiled:
    #         return
    #     if hasattr(torch, 'compile'):
    #         for i in range(len(self.resblocks)):
    #             self.resblocks[i] = torch.compile(self.resblocks[i], mode='reduce-overhead', fullgraph=False)
    #         for i in range(len(self.noise_res)):
    #             self.noise_res[i] = torch.compile(self.noise_res[i], mode='reduce-overhead', fullgraph=False)
    #     self._compiled = True
    ...
    def _forward_compiled(self, x, s, har):
        for i in range(self.num_upsamples):
            ...
            if self.grad_checkpoint:
                x_source = torch.utils.checkpoint.checkpoint(
                    self._noise_conv_block, har, s, i, self.dummy_tensor, use_reentrant=False
                )
            else:
                x_source = self._noise_conv_block(har, s, i, self.dummy_tensor)
            ...
            if self.grad_checkpoint:
                x = torch.utils.checkpoint.checkpoint(
                    self._process_resblocks, x, s, i, self.dummy_tensor, use_reentrant=False
                )
            else:
                x = self._process_resblocks(x, s, i, self.dummy_tensor)
        if self.grad_checkpoint:
            return torch.utils.checkpoint.checkpoint(
                self._generate_output, x, self.dummy_tensor, use_reentrant=False
            )
        else:
            return self._generate_output(x, self.dummy_tensor)
        
    
    def forward(self, x, s, f0):

        if self.dummy_tensor.device != x.device:
            self.dummy_tensor = self.dummy_tensor.to(x.device)
        
        har = self._preprocess_f0(f0)
        result = self._forward_compiled(x, s, har)
        return result
    ...
    def _noise_conv_block(self, har, s, i, dummy_tensor):
        x_source = self.noise_convs[i](har)
        return self.noise_res[i](x_source, s)

    def _generate_output(self, x, dummy_tensor):
        x = F.leaky_relu(x)
        x = self.conv_post(x)
    ...
    def _process_resblocks(self, x, s, i, dummy_tensor):
        xs = None
