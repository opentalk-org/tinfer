...
Buffer pad_columns(const Buffer& source, std::int64_t columns, int fill,
  ...
  const auto source_columns = source.shape[1];
  const auto width = static_cast<std::size_t>(source_columns) * element_size(source.dtype);
  const auto source_pitch = width;
  ...
  if (cudaMemsetAsync(output.data(), fill, output.bytes, stream) != cudaSuccess ||
      cudaMemcpy2DAsync(output.data(), target_pitch, source.data(), source_pitch,
                        width, rows, cudaMemcpyDeviceToDevice, stream) != cudaSuccess) {
...
Output StyleTts2Model::run_cuda(const Batch& batch) const {
  ...
  const auto actual_token_count = static_cast<std::int32_t>(tokens.shape[1]);
  if (backend_ == Backend::TensorRt && actual_token_count < 32) {
    inputs["tokens"] = pad_columns(tokens, 32, 0, device_, stream);
    inputs["mask"] = pad_columns(require(inputs, "mask", DType::Bool), 32, 1, device_, stream);
  }
  const auto token_count = static_cast<std::int32_t>(require(inputs, "tokens", DType::I64).shape[1]);
  ...
  const auto actual_frames = *std::max_element(host_totals.begin(), host_totals.end());
  const auto frames = backend_ == Backend::TensorRt ? std::max(actual_frames, 128) : actual_frames;
  ...
  std::vector<std::int32_t> host_durations(static_cast<std::size_t>(batch_size) * actual_token_count);
  for (std::int32_t item = 0; item < batch_size; ++item) {
    const auto* source = static_cast<const std::int32_t*>(predicted.data()) + static_cast<std::size_t>(item) * token_count;
    auto* target = host_durations.data() + static_cast<std::size_t>(item) * actual_token_count;
    if (cudaMemcpyAsync(target, source, static_cast<std::size_t>(actual_token_count) * sizeof(std::int32_t),
  ...
  output.tensors.push_back(output_tensor("durations", DType::I32, {batch_size, actual_token_count}, host_durations.data(), host_durations.size() * sizeof(std::int32_t)));
