...
use crate::{Alignment, AlignmentItem, AlignmentType, Error, ModelRequest, Result};

use super::text::{MappedItem, PreparedText};

pub(super) fn find<'a>(tensors: &'a [ffi::Tensor], name: &str, dtype: ffi::DType) -> Result<&'a ffi::Tensor> {
    tensors
        .iter()
...
pub(super) fn f32_values(tensor: &ffi::Tensor) -> Result<Vec<f32>> {
...
pub(super) fn i32_values(tensor: &ffi::Tensor) -> Result<Vec<i32>> {
    if !tensor.data.len().is_multiple_of(4) {
        return Err(Error::Inference(format!("native tensor {} has invalid byte length", tensor.name)));
    }
...
pub(super) fn alignment(request: &ModelRequest, text: &PreparedText, durations: &[i32]) -> Option<Alignment> {
