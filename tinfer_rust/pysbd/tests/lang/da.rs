...
#[test]
fn da_matches_upstream() {
    assert_cases("da", false, None, CASES);
}
...
const CASES: &[(&str, &[&str])] = &[
    ...
    ("De lukkede aftalen med Pitt, Briggs & Co. Det lukkede i går.", &["De lukkede aftalen med Pitt, Briggs & Co.", "Det lukkede i går."]),
...
#[test]
fn da_clean_matches_upstream() {
    assert_cases("da", true, None, CLEAN_CASES);
}

#[test]
fn da_pdf_matches_upstream() {
    assert_cases("da", true, Some(DocType::Pdf), PDF_CASES);
}

const CLEAN_CASES: &[(&str, &[&str])] = &[
    (
