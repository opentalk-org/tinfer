...
Bin edges are computed once from the combined enabled and disabled values, using 1 phoneme/s-wide bins spanning the observed floor through ceiling. Both corresponding plots therefore use the same x-axis ranges and bins.
